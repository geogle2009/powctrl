<template>
	<view class="warp">
		...
		<uni-section title="动态加载" type="line">{{messageA[messageA.length-1].content}}</uni-section>
		<view class="example-body">
		...
		<view class="example-body">
			<uni-grid :column="4" border-color="#03a9f4" @change="change">
				<uni-grid-item :index="0">
					<view class="grid-item-box"><image class="image" src="/static/c1.png" mode="aspectFill" /></view>
				</uni-grid-item>
				<uni-grid-item :index="1">
					<view class="grid-item-box"><image class="image" src="/static/c2.png" mode="aspectFill" /></view>
				</uni-grid-item>
				<uni-grid-item :index="2">
					<view class="grid-item-box"><image class="image" src="/static/c3.png" mode="aspectFill" /></view>
				</uni-grid-item>
				<uni-grid-item :index="3">
					<view class="grid-item-box"><image class="image" src="/static/c4.png" mode="aspectFill" /></view>
				</uni-grid-item>
			</uni-grid>
		</view>
	</view>
...
import { uniSection, uniBadge, uniGridItem, uniGrid } from '@dcloudio/uni-ui';
...
export default {
	...
	methods: {
		...
		add() {
			...
			this.sendMessage('aa2',"增加第"+(this.dynamicList.length+1)+"个");
			this.getLastMessage()
		},
		del() {
			this.dynamicList.splice(this.dynamicList.length - 1, 1);
			this.sendMessage('aa2',"删除第"+(this.dynamicList.length)+"个");
			this.getLastMessage()
		},
		sendMessage:function(topic,content){
			var message = new $mqtt.Message(JSON.stringify({ type: 0x10, target: 'To', content: content, extend: '扩展信息' }));
			message.qos = 1;
